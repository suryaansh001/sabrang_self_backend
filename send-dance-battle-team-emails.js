...
async function updateEmailStatus(userId, emailSuccess, emailError = null) {
  try {
    const updateData = {
      ...
      emailSentBy: emailSuccess ? 'dance_battle_team_email_script' : null,
      updatedAt: new Date()
    };
...
async function sendDanceBattleTeamEmails() {
  try {
    console.log('🎭 Starting email sending for DANCE BATTLE team: Quintessence Dance crew (QDC)');
